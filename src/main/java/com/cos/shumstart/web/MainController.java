...
public class MainController {
    ...
    public String main(@AuthenticationPrincipal PrincipalDetails principalDetails,Model model) {
        ...
        if(state == true) {
            Voucher voucher=voucherService.이용권정보(userEntity.getId());
            Rental rental=rentalService.대여정보(userEntity.getId());

            if(voucher==null){//즉 티켓은 만료됐지만 연체해서 빌리고 있는 상황
                voucher=voucherService.연체된이용권정보(userEntity.getId());
            }
